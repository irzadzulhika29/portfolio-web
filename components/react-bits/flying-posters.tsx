...
function autoBind(self: object, { include, exclude }: AutoBindOptions = {}) {
  ...
  for (const [object, key] of getAllProperties(
    Reflect.getPrototypeOf(self) as object
  )) {
    if (key === "constructor" || !filter(key)) continue;

    const descriptor = Reflect.getOwnPropertyDescriptor(object, key);
    if (descriptor && typeof descriptor.value === "function") {
      (self as Record<string | symbol, unknown>)[key] = (
        self as Record<string | symbol, BoundMethod>
      )[key].bind(self);
    }
  }
...
export default function FlyingPosters({
  ...
  const instanceRef = useRef<CanvasScene | null>(null);

  useEffect(() => {
    if (!containerRef.current || !canvasRef.current) return;

    instanceRef.current = new CanvasScene({
